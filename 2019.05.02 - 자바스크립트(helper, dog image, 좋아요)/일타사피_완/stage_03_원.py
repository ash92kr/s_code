...
def play(conn, gameData):
    ...
    power = 115
    wx , wy = gameData.balls[0]
    print('wx:', wx, 'wy: ', wy)
    
    for i in range(1,5):
        if gameData.balls[i] != [0,0]:
            print(gameData.balls[i][0], gameData.balls[i][1])
            x = gameData.balls[i][0]
            y = gameData.balls[i][1]
            dx = abs(wx-x)
            dy = abs(wy-y)
            power = ((x+wx)**2+(y+wy)**2)**0.5 // 10 * 2
            length = (((dx) ** 2 + (dy) ** 2) ** 0.5)
            print('power:', power)
            rad = math.atan2(dx, dy)
            angle = (rad*180)/math.pi + 2

            if wx - x >=  0 and wy - y >= 0:
                angle = angle - 180 - 2
            elif x - wx >=0 and y-wy < 0:
                print('3번쨰 공', angle)
                angle = 180 - angle + 3
            elif wx - x >= 0 and y-wy >= 0:
                angle = angle + 90
